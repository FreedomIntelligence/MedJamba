...
class WeightedRandomSampler(Sampler[int]):
    def __init__(
        ...
        if (
            ...
            or num_samples <= 0
            or num_samples > len(weights)
        ):
            raise ValueError(
                "num_samples should be a positive integer value less than or equal to len(weights), but got num_samples={}".format(
                    num_samples
                )
            )
        ...
        self.rand_list = torch.multinomial(
            self.weights,
    ...
    def update_dynamic_weight(self, new_weights: Sequence[float]):
        ...
        new_samples = torch.multinomial(
            torch.as_tensor(available_weights),
...
class SFT_data(torch.utils.data.Dataset):
    def __init__(self, config, tokenizer):
        ...
        self.datacollatorforseq2seq = transformers.DataCollatorForSeq2Seq(
        ...
        self.roles = ("<|User|>:\n", "<|Assistant|>:\n")
        self.ignore_len = len(
            self.tokenizer.encode(self.sep + self.roles[1], add_special_tokens=False)
        )
        self.debug = True
        ...
        self.data_priority = self.data_priority = {
            "medicalWeb_en_qa": 16,
            "medicalWeb_es_qa": 16,
            "medicalWeb_zh_qa": 16,
    ...
    def preprocess(self, data):
        ...
        for ind, d in enumerate(data):
            if ind % 2 == 1:
                value_ids = self.tokenizer.encode(
                    self.sep + self.roles[1] + d,
                    add_special_tokens=False,
                    max_length=self.config.max_seq_len,
                    truncation=True,
                )
                input_ids += value_ids
                labels += [self.ignore_index] * self.ignore_len + value_ids[
                    self.ignore_len :
                ]
                ...
                input_ids += value_ids
                if len(labels) > 0:
                    labels += [self.tokenizer.eos_token_id] + [self.ignore_index] * (
                        len(value_ids) - 1
                    )
                else:
...
def preprocess(args):
    ...
    sampler = WeightedRandomSampler(
        train_dataset.weights, num_samples=train_dataset.sample_num(), replacement=False
    )
    ...
    for batch_cnt, batch in tqdm(enumerate(train_dataloader)):
        ...
        all_labels.append(cur_label)
        assert (
            len(cur_input) == len(cur_label) == args.max_seq_len
        ), f"{len(cur_input)},{len(cur_label)}"
    ...
    save_dataset = datasets.Dataset.from_dict(
        {"input_ids": all_inputs_ids, "labels": all_labels}
    )
    ...
    table.add_data(
        json.dumps(train_dataset.data_priority, ensure_ascii=False, indent=2),
        json.dumps(train_dataset.data_epoch, ensure_ascii=False, indent=2),
        json.dumps(train_dataset.get_data_info(), ensure_ascii=False, indent=2),
    )
